...
<body>
    ...
    <div class="vernav2 iconmenu">
    	<ul>
			...
			<li><a href="#member" class="editor">会员管理</a>
            	<span class="arrow"></span>
            	<ul id="member">
...
<div class="panel panel-default">
	<div class="panel-heading">
		...
		<a href="<?php echo U('add');?>" class="btn btn-success"><span class="glyphicon glyphicon-plus"></span> 添加管理员</a>
	...
	<table class="table-bordered table table-hover">
		...
		<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$admin): $mod = ($i % 2 );++$i;?><tr>
			...
			<?php if($admin['logintime']): ?><td><?php echo date('Y-m-d H:i:s',$admin['logintime']);?></td>
			<?php else: ?>
				<td>该用户未进行登录</td><?php endif; ?>
			<td><?php echo ($admin['loginnum']); ?></td>
			<?php if($admin[statu] == 0): ?><td><span class="btn btn-success">开启</span></td>
			<?php else: ?>
